...
class WebPDataset(Dataset):
    def __init__(self, csv_file, transform=None,  with_augmentation = False):
        ...
        self.transform_w_aug = transforms.Compose([
            ...
            transforms.ColorJitter(brightness=0.2, contrast=0.2),  # Random changes in brightness, contrast, saturation, and hue
    ...
    def __getitem__(self, idx):
        while True:
            try:
                ...
                # if "ff24a170-5f83-4a4c-a653-287b99efb01e" in img_name:
                #     continue
                ...
                image = cv.merge([gray_image, gray_image, gray_image]) 
                ...
                label = torch.tensor(1) if self.data.iloc[idx, 1] == 1 else torch.tensor(0)
...
class CustomModel(nn.Module):
    def __init__(self):
        ...
        resnet = models.resnet50(pretrained=True)  # Do not load default weights
...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        # p = torch.sigmoid(inputs)  # Sigmoid outputs probabilities
        # BCE_loss = nn.BCELoss(reduction='none')(p, targets)
        # p = F.softmax(inputs, dim=1)
        p = inputs
        CE_loss = nn.CrossEntropyLoss()(p, targets)
        # Calculate pt
        pt = p * targets + (1 - p) * (1 - targets)  # This calculates the probabilities for the true class
...
def test_model(model, test_loader, fold):
    ...
    checkpoint_path = os.path.join(sftp_utils.sftp_path,f'media/Data/2d_annot_model_lbc/dataset/checkpoint_folder_lbc_resnet18_SA_rectfd_2_gray_resnet50_focal_loss/best_model_fold_{fold}.pth')
    ...
    with torch.no_grad():
        for image, label in test_loader:
            ...
            probabilities = F.softmax(outputs, dim=1) 
            # preds = (probabilities >= 0.5).float()  # Convert probabilities to predictions using 0.5 as threshold
            preds = F.softmax(probabilities, dim=1)
            _, preds = torch.max(outputs, 1)
            # preds = (probabilities >= 0.5).float()  # Convert probabilities to predictions using 0.5 as threshold
            # print("label=",label,"pred=",preds)
...
checkpoint_folder = os.path.join(sftp_utils.sftp_path,'media/Data/2d_annot_model_lbc/dataset/checkpoint_folder_lbc_resnet18_SA_rectfd_2_gray_resnet50_focal_loss')
...
metrics_all_folds_df.to_csv(os.path.join(checkpoint_folder, 'metrics_all_folds_shuffel_gray.csv'), index=False)