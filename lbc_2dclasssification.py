...
class CustomModel(nn.Module):
    def __init__(self):
        ...
        self.backbone = nn.Sequential(*list(resnet.children())[:-1])  # Remove the last fully connected layer and avgpool layer
        ...
        # self.additional_layers = nn.Sequential(
        #     nn.Conv2d(2048, 1024, kernel_size=1, padding=1),
        #     nn.BatchNorm2d(1024),
        #     nn.ReLU(),
...
class CancerClassifier:
    ...
    def train(self, train_loader, val_loader, num_epochs, learning_rate, fold, latest_epoch, checkpoint_folder=None, resume=False, early_stopping_patience=5):
...
def test_model(model, test_loader, fold):
    ...
    checkpoint_path = os.path.join(sftp_utils.sftp_path,f'media/dataset/annot_cases_cpap/data/checkpoint_folder_cpap_resnet50_ft_aug/best_model_fold_{fold}.pth')
...
# Main execution
dataset_folder = os.path.join(sftp_utils.sftp_path,'media/dataset/annot_cases_cpap/data/train_folds_otsu')
checkpoint_folder = os.path.join(sftp_utils.sftp_path,'media/dataset/annot_cases_cpap/data/checkpoint_folder_cpap_resnet50_ft_aug')
